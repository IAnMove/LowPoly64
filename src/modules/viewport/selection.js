  ...
  if (state.bonesVisible) {
    ...
    if (pivotFromBone) {
      ...
      if (event.shiftKey && state.selectedMesh && state.selectedMesh.userData.isPivot && pivotFromBone !== state.selectedMesh) {
        emit('bone:attach', pivotFromBone);
        return;
      }
...
export function selectMesh(mesh) {
  ...
  emit('animation:show-timeline', mesh);
}
...
// Listen for proxy position updates from scene.js (avoids circular import)
on('proxy:update-position', () => updateProxyPosition());
