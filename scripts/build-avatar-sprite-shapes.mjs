...
function buildRadialContour(png, stats, bounds) {
  ...
  const cx = crop.x + (crop.w * 0.5);
  const cy = crop.y + (crop.h * 0.5);
  const bins = Array.from({ length: CONTOUR_POINT_COUNT }, () => null);

  stats.boundary.forEach((point) => {
    const dx = point.x - cx;
    const dy = point.y - cy;
    const angle = Math.atan2(dy, dx);
    const normalizedAngle = (angle + Math.PI) / (Math.PI * 2);
    const bin = Math.min(CONTOUR_POINT_COUNT - 1, Math.max(0, Math.floor(normalizedAngle * CONTOUR_POINT_COUNT)));
    const distance = Math.hypot(dx, dy);
    if (!bins[bin] || distance > bins[bin].distance) {
      bins[bin] = { x: point.x, y: point.y, distance };
    }
  });
  ...
  return bins.map((entry, index) => {
    if (entry) {
      return [
        Number(((entry.x - crop.x) / crop.w).toFixed(4)),
        Number(((entry.y - crop.y) / crop.h).toFixed(4)),
      ];
    }
    const angle = -Math.PI + (((index + 0.5) / CONTOUR_POINT_COUNT) * Math.PI * 2);
    return [
      Number((0.5 + (Math.cos(angle) * 0.5)).toFixed(4)),
      Number((0.5 + (Math.sin(angle) * 0.5)).toFixed(4)),
    ];
  });
}
