...
- **PNG to Flat Model**: turn a transparent PNG/WebP silhouette into a textured 2.5D mesh and paint local inflate/deflate corrections
...
2. Load a PNG or WebP with a transparent background.
3. Adjust density, thickness, bulge, alpha cut, smoothing, and side color.
4. Paint red with **INFLATE** where more volume is needed or blue with **DEFLATE** where the automatic result is too thick.
5. Orbit the live 3D preview and click **CREATE MODEL**.
6. Select the derived group and use **EDIT PNG MODEL** to reopen its source and paint map.

The result is a closed 2.5D reconstruction from one view, not inferred hidden
anatomy. It supports undo/redo, scene save/load, editable object JSON, and the
normal GLB export path. See [docs/png-to-flat-model.md](docs/png-to-flat-model.md)
for controls, input advice, limits, and troubleshooting.
