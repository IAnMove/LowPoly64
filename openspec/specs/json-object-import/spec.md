...
Additionally, the system SHALL detect and accept the `CharacterModel` format: `{ name, archetype, slots: [{ slotId, pieces }], animationProfile?, skeletonId? }`. When this format is detected, the system SHALL validate the archetype and slots, convert the CharacterModel to the internal `pieces[]` format, store archetype metadata in the group's `userData`, and resolve the skeleton and animation profile when present.
...
- **THEN** the system SHALL convert it to the internal format, apply skeleton and animation profile data, and load the model with archetype metadata in `userData`

#### Scenario: Import old format still works
- **WHEN** the user pastes a JSON with `pieces[]` but no `archetype` field
- **THEN** the system SHALL import it through the existing legacy flow without behavior changes
...
For CharacterModel format, the system SHALL additionally validate that `archetype` is registered, each slot `slotId` is valid for that archetype, and each slot piece provides `template`, `name`, `size`, `offset`, and `material`.
...
- **WHEN** a CharacterModel JSON has `archetype: "DRAGON"` and that archetype is not registered
- **THEN** the system SHALL display an error indicating the archetype is not recognized

#### Scenario: Invalid slot for archetype
- **WHEN** a CharacterModel with archetype `"CAR"` includes a slot with `slotId: "HEAD"`
- **THEN** the system SHALL display an error indicating that `HEAD` is not a valid slot for `CAR`
...
The `ask.md` SHALL document CharacterModel as the preferred format for characters and vehicles, while keeping the old object format available as a legacy alternative.
