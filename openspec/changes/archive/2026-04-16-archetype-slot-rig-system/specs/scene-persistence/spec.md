...
When serializing groups that have CharacterModel metadata (`userData.archetype`, `userData.slotMap`, etc.), the system SHALL include this metadata in the serialized data so it can be restored on load.
...
- **THEN** the serialized data SHALL include `archetype`, `slotMap`, `animationProfile`, `skeletonId`, and `slotBindings` in the group's serialized object
...
When loading groups that have serialized CharacterModel metadata, the system SHALL restore `userData.archetype`, `userData.slotMap`, `userData.animationProfile`, `userData.skeletonId`, and `userData.slotBindings`.
...
- **THEN** the loaded groups SHALL have `userData.archetype`, `userData.slotMap`, and other CharacterModel fields restored, enabling the Rig panel to function
...
When the group has CharacterModel metadata, `serializeGroupAsImportJSON` SHALL be able to produce either the old flat format or the new CharacterModel format (with archetype/slots), based on a parameter.
...
- **WHEN** a group with archetype metadata is exported with the CharacterModel format option
- **THEN** the output SHALL use `{ name, archetype, slots: [...], animationProfile }` format instead of flat `pieces[]`
