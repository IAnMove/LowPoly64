...
The system SHALL support nesting up to 4 levels deep (root → parent → child → grandchild). Deeper nesting SHALL be ignored with a console warning.

#### Scenario: Three levels of nesting
- **WHEN** pieces define `TORSO` → `BRAZO_IZQ` (parent: TORSO) → `MANO_IZQ` (parent: BRAZO_IZQ)
